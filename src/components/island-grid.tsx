...
const generateRandomGrid = (): CellState[][] => {
  return Array.from({ length: ROWS }, () =>
    Array.from({ length: COLS }, () => (Math.random() > 0.55 ? "land" : "water"))
  );
};
...
const IslandGrid = () => {
  ...
  const countIslands = () => {
    ...
    const dfs = (r: number, c: number, colorIndex: number, currentIsland: [number, number][]) => {
      if (
        r < 0 || r >= ROWS || c < 0 || c >= COLS ||
        resultGrid[r][c] !== "land" || visited[r][c]
      ) return;

      visited[r][c] = true;
      currentIsland.push([r, c]);
      resultGrid[r][c] = colorIndex;
  ...
  const getCellClass = (cell: CellState) => {
    ...
    if (typeof cell === "number") return ISLAND_COLORS[cell % ISLAND_COLORS.length];
    return "";
  };
  ...
  return (
    <section className="space-y-5">
      ...
      <p className="text-sm text-zinc-500 dark:text-zinc-400">
        Click vào ô để thêm/xoá đất. Nhấn &quot;Đếm đảo&quot; để chạy thuật
        toán DFS.
      </p>
              ...
              <ul className="space-y-1.5">
                {islandCoords.map((coords, i) => (
                  <li key={i} className="text-sm">
                    <span className={cn("font-semibold", ISLAND_TEXT_COLORS[i % ISLAND_TEXT_COLORS.length])}>
                      Đảo {i + 1}:
                    </span>{" "}
